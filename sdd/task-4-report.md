...
## Fix follow-up
- Enabled decorator support in `tsconfig.json` with `experimentalDecorators` and `emitDecoratorMetadata`.
- Updated `TraceBamlOperation` to tolerate both legacy and stage-3 decorator invocation so Vitest/typecheck and the Nub/tsx runtime both work.

## Validation
- `BAML_LOG=warn COPILOT_PROXY_BASE_URL=http://127.0.0.1:8080/v1 COPILOT_PROXY_API_KEY=anything OTEL_SDK_DISABLED=true nub run council decision-council run --smoke --input examples/smoke-question.md --output runs/fix-smoke` ✅
