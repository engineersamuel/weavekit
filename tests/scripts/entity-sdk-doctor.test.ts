...
import {
  ...
  runEntitySdkDoctor,
} from "../../scripts/entity-sdk-doctor.js";
...
async function createRepoWithSkillBackedPersona(): Promise<string> {
  ...
  await write(
    root,
    "entities/personas/mckinsey-strategist.yaml",
    `
...
`,
  ...
  await write(
    root,
    ".copilot/skills/mckinsey-strategist/SKILL.md",
    "---\nname: mckinsey-strategist\n---\n# Skill\n",
  );
  await write(
    root,
    ".copilot/skills/other-skill/SKILL.md",
    "---\nname: other-skill\n---\n# Skill\n",
  );
  return root;
}

async function createHveCorePluginConfig(
  root: string,
): Promise<{ configPath: string; pluginDir: string }> {
...
describe("entity SDK doctor", () => {
  it("resolves the Copilot CLI binary from the SDK package layout", async () => {
    ...
    await write(
      root,
      "node_modules/.nub/@github+copilot-sdk@1.0.4-a/node_modules/@github/copilot-sdk/dist/index.js",
      "",
    );
    await write(
      root,
...
`,
    ...
    await expect(
      ...
      `Configured plugin directory for hve-core does not exist: ${missingPluginDir}`,
    );
