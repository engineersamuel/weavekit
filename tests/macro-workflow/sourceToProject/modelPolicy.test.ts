...
describe("source-to-project model policy", () => {
  it("routes default operations to contextual models", () => {
    expect(
      sourceToProjectModelDecision(SourceToProjectModelOperation.SOURCE_READING),
    ).toMatchObject({
      model: "gpt-5.5",
    });
    expect(
      sourceToProjectModelDecision(SourceToProjectModelOperation.PLAN_DISTILLATION),
    ).toMatchObject({
      model: "gpt-5-mini",
    });
    expect(
      sourceToProjectModelDecision(SourceToProjectModelOperation.PLAN_GENERATION),
    ).toMatchObject({
    ...
    expect(
      sourceToProjectModelDecision(SourceToProjectModelOperation.IMPLEMENTATION),
    ).toMatchObject({
      model: "gpt-5.3-codex",
    });
    expect(
      sourceToProjectModelDecision(SourceToProjectModelOperation.IMPLEMENTATION_FIX),
    ).toMatchObject({
      model: "gpt-5.5",
    });
    expect(
      sourceToProjectModelDecision(SourceToProjectModelOperation.IMPLEMENTATION_REVIEW),
    ).toMatchObject({
      model: "gpt-5.5",
    });
    expect(
      sourceToProjectModelDecision(SourceToProjectModelOperation.WORKFLOW_PLANNING),
    ).toMatchObject({
      model: "claude-opus-4.8",
    });
  ...
  it("applies source_to_project.copilot_model only to Copilot SDK decisions", () => {
    ...
    expect(
      sourceToProjectCopilotModelDecision(SourceToProjectModelOperation.SOURCE_READING, config),
    ).toMatchObject({
      model: "copilot-override",
    });
    expect(
      sourceToProjectCopilotModelDecision(SourceToProjectModelOperation.PROJECT_RESEARCH, config),
    ).toMatchObject({
      model: "copilot-override",
    });
    expect(
      sourceToProjectBamlRoute(SourceToProjectModelOperation.SOURCE_READING, {}),
    ).toMatchObject({
      model: "gpt-5-mini",
  ...
  it("routes target project Copilot SDK research to Sonnet while BAML project distillation uses mini", () => {
    expect(
      sourceToProjectCopilotModelDecision(SourceToProjectModelOperation.PROJECT_RESEARCH),
    ).toMatchObject({
  ...
  it("uses execution-route metadata for source-to-project node cards", () => {
    ...
    expect(
      sourceToProjectNodeModelMetadata(SourceToProjectModelOperation.SOURCE_READING, options),
    ).toMatchObject({
      model: "copilot-override",
    });
    expect(
      sourceToProjectNodeModelMetadata(SourceToProjectModelOperation.PLAN_GENERATION, options),
    ).toMatchObject({
      model: "copilot-override",
    });
    expect(
      sourceToProjectNodeModelMetadata(SourceToProjectModelOperation.OPPORTUNITY_MAPPING, options),
    ).toMatchObject({
      model: "claude-opus-4.8",
    });
    expect(
    ...
    expect(
      sourceToProjectNodeModelMetadata(SourceToProjectModelOperation.DETERMINISTIC, options),
    ).toMatchObject({
      model: "deterministic",
    });
  });
  ...
  it("maps known BAML_MODEL values to generated clients and marks unknown values for runtime registry routing", () => {
    expect(
      sourceToProjectBamlRoute(SourceToProjectModelOperation.PLAN_DISTILLATION, {
    ...
    expect(
      sourceToProjectBamlRoute(SourceToProjectModelOperation.WORKFLOW_PLANNING, {
        BAML_MODEL: "claude-opus-4.8",
      }),
    ).toMatchObject({
      model: "claude-opus-4.8",
  ...
  it("does not let the blanket BAML_MODEL override weaken semantic compiler gates", () => {
    const env = { BAML_MODEL: "gpt-5-mini" };

    expect(
      sourceToProjectBamlRoute(SourceToProjectModelOperation.OPPORTUNITY_MAPPING, env),
    ).toMatchObject({
      model: "claude-opus-4.8",
      client: "CopilotProxyClaudeOpus48",
    });
    expect(
      sourceToProjectBamlRoute(SourceToProjectModelOperation.PORTFOLIO_AUDIT, env),
    ).toMatchObject({
      model: "gpt-5.5",
      client: "CopilotProxyGpt55",
    });
    expect(
      sourceToProjectBamlRoute(SourceToProjectModelOperation.PORTFOLIO_REPAIR, env),
    ).toMatchObject({
      model: "gpt-5.5",
      client: "CopilotProxyGpt55",
    });
  ...
  it("maps every source-to-project BAML function to the client for its model policy", () => {
    ...
    expect(sourceToProjectBamlFunctionRoute("DistillProjectApplicability")).toMatchObject({
      model: "gpt-5.5",
      client: "CopilotProxyGpt55",
    });
    expect(sourceToProjectBamlFunctionRoute("RepairProjectApplicability")).toMatchObject({
      model: "gpt-5.5",
      client: "CopilotProxyGpt55",
    });
    ...
    expect(sourceToProjectBamlFunctionRoute("DistillPortfolioPlanDraft")).toMatchObject({
      model: "gpt-5-mini",
      client: "CopilotProxyGpt5Mini",
    });
    expect(sourceToProjectBamlFunctionRoute("AuditPortfolioCoverage")).toMatchObject({
      model: "gpt-5.5",
      client: "CopilotProxyGpt55",
    });
    expect(sourceToProjectBamlFunctionRoute("RepairPortfolioPlan")).toMatchObject({
      model: "gpt-5.5",
      client: "CopilotProxyGpt55",
    });
