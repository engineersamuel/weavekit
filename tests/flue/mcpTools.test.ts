...
describe("connectConfiguredMcpTools", () => {
  it("connects each remote spec and aggregates tools", async () => {
    ...
    expect(connect).toHaveBeenCalledWith("EngHub", {
      url: "https://mcp.eng.ms",
      transport: "streamable-http",
    });
    ...
    const connect = vi.fn(
      async (name: string) =>
        ({
          name,
          tools: [],
          close: name === "EngHub" ? closeA : closeB,
        }) satisfies McpServerConnection,
    );
  ...
  it("skips disabled specs and does not connect to them", async () => {
    ...
    const connect = vi.fn(
      async (name: string) =>
        ({ name, tools: [], close: vi.fn(async () => undefined) }) satisfies McpServerConnection,
    );
  ...
  it("preserves async Promise<void> signature for wrapped close function", async () => {
    ...
    const connect = vi.fn(
      async (name: string) =>
        ({ name, tools: [], close: innerClose }) satisfies McpServerConnection,
    );
