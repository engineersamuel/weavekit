...
- **Always use oxlint for linting and oxfmt for formatting.** Run `nub run lint` (`oxlint --deny-warnings .`) before committing; do not introduce ESLint, Prettier, Biome, or other lint/format tools. Use `nub run fmt` (`oxfmt`) on new files you create. `mise run pre-commit` auto-fixes staged files with oxlint and auto-formats newly *added* staged files with oxfmt (install the hook once per clone with `mise generate git-pre-commit --write`). Note: this repo has not been fully reformatted with oxfmt yet, so CI and the pre-commit hook only enforce formatting on newly added files, not edits to pre-existing files — oxfmt reformats whole files, so touching one line of an unformatted legacy file would otherwise produce a huge unrelated diff.
