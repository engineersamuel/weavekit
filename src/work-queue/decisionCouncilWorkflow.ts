...
export async function createDecisionCouncilBeadsWorkflow(args: {
  ...
  return runWorkQueueSpan("create-workflow", { itemId: "generated" }, async () => {
    ...
    const runItem = await runWorkQueueSpan("create-workflow-item", { itemId: "run-council" }, async () =>
      backend.create({
    ...
    const reviewItem = await runWorkQueueSpan("create-workflow-item", { itemId: "review-report" }, async () =>
      backend.create({
    ...
    const followUpItem = await runWorkQueueSpan("create-workflow-item", { itemId: "implement-next-experiment" }, async () =>
      backend.create({
