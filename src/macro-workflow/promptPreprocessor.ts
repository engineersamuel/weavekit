    ...
    new Promise<XPostFetchResult>((resolve, reject) => {
      ...
      child.on("error", (error: NodeJS.ErrnoException) => {
        settle(() => {
          if (error.code === "ENOENT") {
            reject(
              new Error(
                `Could not find grok on PATH while fetching ${url}. Install grok or make it available on PATH.`,
              ),
            );
            return;
          }
      ...
      child.on("close", (code, signal) => {
        settle(() => {
          if (code !== 0) {
            const details =
              stderr.trim() || (signal ? `terminated by signal ${signal}` : "no stderr output");
            reject(
              new Error(
                `grok failed while fetching ${url} with exit code ${code ?? "unknown"}: ${details}`,
              ),
            );
            return;
          }
...
function appendFetchedXPosts(prompt: string, fetchedXPosts: XPostFetchResult[]): string {
  const sections = fetchedXPosts.flatMap((post) => [`### ${post.url}`, "", post.markdown]);
  const resolvedSection = ["## Resolved X Post Sources", "", ...sections].join("\n");
...
function stripTrailingUrlPunctuation(rawUrl: string): string {
  let url = rawUrl;
  while (url.length > 0) {
    const stripped = url.replace(/[.,;:!?]+$/u, "").replace(/[\])}]+$/u, "");
    if (stripped === url) {
      return url;
    }
    url = stripped;
